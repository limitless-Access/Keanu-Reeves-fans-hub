...
async function validateAndUseAccessCode(code, usedBy='') {
  ...
  if (!clean) return {valid:false,reason:'missing'};
  const data = await loadFromJsonBin();
  const index = data.accessCodes.findIndex(x => String(x.code || '').toUpperCase() === clean);
  if (index < 0) return {valid:false,reason:'invalid'};
  if (data.accessCodes[index].used) return {valid:false,reason:'used'};
  data.accessCodes[index] = {...data.accessCodes[index],used:true,usedBy:usedBy||'',usedAt:new Date().toISOString()};
  await saveHubData(data);
  return {valid:true,record:data.accessCodes[index]};
}
